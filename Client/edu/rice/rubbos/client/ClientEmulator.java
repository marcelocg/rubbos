...
public class ClientEmulator {
  ...
  /**
   * Creates a new <code>ClientEmulator</code> instance. The program is stopped
  ...
  public ClientEmulator() {
    ...
    urlGen = rubbos.getURLGen();
    if (urlGen == null)
      Runtime.getRuntime().exit(1);
    // Check that the transition table is ok and print it
//    TransitionTable transition = new TransitionTable(rubbos.getNbOfColumns(),
//        rubbos.getNbOfRows(), null, rubbos.useTPCWThinkTime());
...
//      transition.displayMatrix("Author");
  ...
  public static void main(String[] args) {
    ...
    ClientEmulator client = new ClientEmulator(); // Get also rubbos.properties
                                                  // info
    ...
    if (isMainClient) {
      ...
      // index.html file
      System.out
          .println("RUBBoS client emulator - (C) Rice University/INRIA 2001\n");
      reportDir = "bench/w" + client.rubbos.getNbOfClients() + "s" + client.rubbos.getNbOfServletServers() + "/";
      ...
      System.out
          .println("<h2>RUBBoS client emulator - (C) Rice University/INRIA 2001</h2><p>\n");
      ...
      System.out
          .println("RUBBoS remote client emulator - (C) Rice University/INRIA 2001\n");
      startDate = new GregorianCalendar();
    }

    System.out.println("<p><hr><p>");
    
    client.rubbos.checkPropertiesFile();
    
    if (isMainClient) {
      // Start remote clients
      System.out.println("Total number of clients for this experiment: "
          + (client.rubbos.getNbOfClients() * (client.rubbos.getRemoteClients()
              .size() + 1)) + "<br>");
      remoteClient = new Process[client.rubbos.getRemoteClients().size()];
      for (int i = 0; i < client.rubbos.getRemoteClients().size(); i++) {
        try {
          System.out.println("ClientEmulator: Starting remote client on "
              + client.rubbos.getRemoteClients().get(i) + "<br>\n");
          ...
          rcmdClient[3] = client.rubbos.getClientsRemoteCommand() + " "
              + tmpDir + "trace_client" + (i + 1) + ".html " + tmpDir
              + "stat_client" + (i + 1) + ".html";
          remoteClient[i] = Runtime.getRuntime().exec(rcmdClient);
          System.out.println("&nbsp &nbsp Command is: " + rcmdClient[0] + " "
              + rcmdClient[1] + " " + rcmdClient[2] + " " + rcmdClient[3]
              + "<br>\n");
        } catch (IOException ioe) {
          System.out.println("An error occured while executing remote client ("
              + ioe.getMessage() + ")");
        }
      }
      ...
      System.out
          .println("<CENTER></A><A NAME=\"trace\"><h2>*** Monitoring ***</h2></CENTER>");
      try {
        ...
        try {
          ...
          System.out.println("&nbsp &nbsp Command is: " + delFiles[0] + " "
              + delFiles[1] + " " + delFiles[2] + " " + delFiles[3] + "<br>\n");
          ...
          if (client.rubbos.getCJDBCServerName() != null
            ...
            System.out.println("&nbsp &nbsp Command is: " + delFiles[0] + " "
                + delFiles[1] + " " + delFiles[2] + " " + delFiles[3]
                + "<br>\n");
            delProcess = Runtime.getRuntime().exec(delFiles);
            delProcess.waitFor();
          }
        ...
        // Monitor Web server
        int fullTimeInSec = (client.rubbos.getUpRampTime()
            + client.rubbos.getSessionTime() + client.rubbos.getDownRampTime()) / 1000 + 5; // Give
                                                                                            // 5
                                                                                            // seconds
                                                                                            // extra
                                                                                            // for
                                                                                            // init
        System.out
            .println("ClientEmulator: Starting monitoring program on Web server "
                + client.rubbos.getWebServerName() + "<br>\n");
        ...
        cmdWeb[3] = "/bin/bash";
        cmdWeb[4] = "-c";
        cmdWeb[5] = "'LANG=en_GB.UTF-8 " + client.rubbos.getMonitoringProgram()
            + " " + client.rubbos.getMonitoringOptions() + " "
            + client.rubbos.getMonitoringSampling() + " " + fullTimeInSec
            + " -o " + tmpDir + "web_server'";
        webServerMonitor = Runtime.getRuntime().exec(cmdWeb);
        System.out.println("&nbsp &nbsp Command is: " + cmdWeb[0] + " "
            + cmdWeb[1] + " " + cmdWeb[2] + " " + cmdWeb[3] + " " + cmdWeb[4]
            + " " + cmdWeb[5] + "<br>\n");
        ...
        if (client.rubbos.getCJDBCServerName() != null
            && !client.rubbos.getCJDBCServerName().equals("")) {
          System.out
              .println("ClientEmulator: Starting monitoring program on CJDBC server "
                  + client.rubbos.getCJDBCServerName() + "<br>\n");
          cmdWeb[0] = client.rubbos.getMonitoringRsh();
          cmdWeb[1] = "-x";
          cmdWeb[2] = client.rubbos.getCJDBCServerName();
          cmdWeb[3] = "/bin/bash";
          cmdWeb[4] = "-c";
          cmdWeb[5] = "'LANG=en_GB.UTF-8 "
              + client.rubbos.getMonitoringProgram() + " "
              + client.rubbos.getMonitoringOptions() + " "
              + client.rubbos.getMonitoringSampling() + " " + fullTimeInSec
              + " -o " + tmpDir + "cjdbc_server'";
        ...
        System.out
            .println("ClientEmulator: Starting monitoring program on Database server "
        ...
        cmdDB[3] = "/bin/bash";
        cmdDB[4] = "-c";
        cmdDB[5] = "'LANG=en_GB.UTF-8 " + client.rubbos.getMonitoringProgram()
            + " " + client.rubbos.getMonitoringOptions() + " "
            + client.rubbos.getMonitoringSampling() + " " + fullTimeInSec
            + " -o " + tmpDir + "db_server'";
        dbServerMonitor = Runtime.getRuntime().exec(cmdDB);
        System.out.println("&nbsp &nbsp Command is: " + cmdDB[0] + " "
            + cmdDB[1] + " " + cmdDB[2] + " " + cmdDB[3] + " " + cmdDB[4] + " "
            + cmdDB[5] + "<br>\n");

        // Monitor local client
        System.out
            .println("ClientEmulator: Starting monitoring program locally on client<br>\n");
        ...
        cmdClient[4] = "-c";
        cmdClient[5] = "'LANG=en_GB.UTF-8 "
            + client.rubbos.getMonitoringProgram() + " "
            + client.rubbos.getMonitoringOptions() + " "
            + client.rubbos.getMonitoringSampling() + " " + fullTimeInSec
            + " -o " + reportDir + "client0'";
        clientMonitor = Runtime.getRuntime().exec(cmdClient);
        System.out.println("&nbsp &nbsp Command is: " + cmdClient[0] + " "
            + cmdClient[1] + " " + cmdClient[2] + " " + cmdClient[3] + " "
            + cmdClient[4] + " " + cmdClient[5] + "<br>\n");

        remoteClientMonitor = new Process[client.rubbos.getRemoteClients()
            .size()];
        ...
        System.out
            .println("An error occured while executing monitoring program ("
                + ioe.getMessage() + ")");
      }
    ...
    for (int i = 0; i < client.rubbos.getRemoteClients().size(); i++)
      System.out.println("<A HREF=\"trace_client" + (i + 1)
          + ".html\">client1 (" + client.rubbos.getRemoteClients().get(i)
          + ") traces</A><br>");
    ...
    stats.reset(); // Note that as this is not atomic we may lose some stats
                   // here ...
    ...
    for (int i = 0; i < client.rubbos.getRemoteClients().size(); i++)
      System.out.println("<A HREF=\"stat_client" + (i + 1)
          + ".html\">remote client (" + client.rubbos.getRemoteClients().get(i)
          + ") statistics</A><br>");
    ...
    System.out.println("<TR><TD><B>Up ramp length</B><TD>"
        ...
        + client.rubbos.getUpRampTime() + " ms)");
    ...
    upRampStats.display_stats("Up ramp",
        TimeManagement.diffTimeInMs(upRampDate, runSessionDate), false);
    System.out.println("<br><A NAME=\"run_stat\"></A>");
    runSessionStats.display_stats("Runtime session",
        TimeManagement.diffTimeInMs(runSessionDate, downRampDate), false);
    System.out.println("<br><A NAME=\"down_stat\"></A>");
    downRampStats.display_stats("Down ramp",
        TimeManagement.diffTimeInMs(downRampDate, endDownRampDate), false);
    System.out.println("<br><A NAME=\"all_stat\"></A>");
    allStats.display_stats("Overall",
        TimeManagement.diffTimeInMs(upRampDate, endDownRampDate), false);
    ...
    if (isMainClient) {
      ...
      try {
        ...
        System.out.println("Waiting for webserver monitor to complete...");
        webServerMonitor.waitFor();
        System.out
            .println("Waiting for Database server monitor to complete...");
        dbServerMonitor.waitFor();
        System.out
            .println("Ok, monitors have completed their work. Going on...");
        System.out.flush();
        ...
        System.out
            .println("An error occured while waiting for remote processes termination ("
                + e.getMessage() + ")");
      }
      ...
      try {
        ...
        scpCmd[1] = client.rubbos.getWebServerName() + ":" + tmpDir
            + "/web_server";
        System.out.println("Copying webserver sar file...");
        System.out.println(scpCmd[0] + " " + scpCmd[1] + " " + scpCmd[2]);
        System.out.flush();
        p = Runtime.getRuntime().exec(scpCmd);
        ...
        if (client.rubbos.getCJDBCServerName() != null
            && !client.rubbos.getCJDBCServerName().equals("")) {
          scpCmd[1] = client.rubbos.getCJDBCServerName() + ":" + tmpDir
              + "/cjdbc_server";
          p = Runtime.getRuntime().exec(scpCmd);
          p.waitFor();
        }
        scpCmd[1] = client.rubbos.getDBServerName() + ":" + tmpDir
            + "/db_server";
        p = Runtime.getRuntime().exec(scpCmd);
        System.out.println("Copying database server sar file...");
        // any error message?
        errorGobbler = new StreamGobbler(p.getErrorStream(), "ERROR");            
        ...
        System.out.println("An error occured while scping the files over ("
            + e.getMessage() + ")");
      }
      ...
      try {
        ...
        Process p;
        // Web server
        cmd = "mv " + reportDir + "/" + "web_server " + reportDir + "/"
            + "web_server.bin";
        ...
        if (client.rubbos.getCJDBCServerName() != null
            && !client.rubbos.getCJDBCServerName().equals("")) {
          cmd = "mv " + reportDir + "/" + "cjdbc_server " + reportDir + "/"
              + "cjdbc_server.bin";
          p = Runtime.getRuntime().exec(cmd);
          p.waitFor();
        }
        ...
        // Database Server
        cmd = "mv " + reportDir + "/" + "db_server " + reportDir + "/"
            + "db_server.bin";
        p = Runtime.getRuntime().exec(cmd);
        p.waitFor();

        // Localhost
        cmd = "mv " + reportDir + "/" + "client0 " + reportDir + "/"
            + "client0.bin";
        p = Runtime.getRuntime().exec(cmd);
        p.waitFor();
        ...
        // Remote clients
        for (int i = 0; i < client.rubbos.getRemoteClients().size(); i++) {
          cmd = "mv " + reportDir + "/" + "client" + (i + 1) + " " + reportDir
              + "/" + "client" + (i + 1) + ".bin";
          p = Runtime.getRuntime().exec(cmd);
          p.waitFor();
        }
        ...
        // All files rename at this point in time. Time to go forth and convert
        // them into ascii
        ...
        convCmd[2] = "/bin/bash";
        convCmd[3] = "-c";
        convCmd[4] = common + "web_server.bin > " + reportDir + ""
            + "web_server'";
        System.out.println("&nbsp &nbsp Command is: " + convCmd[0] + " "
            + convCmd[1] + " " + convCmd[2] + " " + convCmd[3] + " "
            + convCmd[4] + "<br>\n");
        p = Runtime.getRuntime().exec(convCmd);

        if (client.rubbos.getCJDBCServerName() != null
            && !client.rubbos.getCJDBCServerName().equals("")) {
          convCmd[4] = common + "cjdbc_server.bin > " + reportDir + ""
              + "cjdbc_server'";
          System.out.println("&nbsp &nbsp Command is: " + convCmd[0] + " "
              + convCmd[1] + " " + convCmd[2] + " " + convCmd[3] + " "
              + convCmd[4] + "<br>\n");
        ...
        convCmd[4] = common + "db_server.bin > " + reportDir + ""
            + "db_server'";
        System.out.println("&nbsp &nbsp Command is: " + convCmd[0] + " "
            + convCmd[1] + " " + convCmd[2] + " " + convCmd[3] + " "
            + convCmd[4] + "<br>\n");
        ...
        for (int i = 0; i < client.rubbos.getRemoteClients().size(); i++) {
          convCmd[2] = common + "client" + (i + 1) + ".bin > " + reportDir + ""
              + "client" + (i + 1) + "'";
          System.out.println("&nbsp &nbsp Command is: " + convCmd[0] + " "
              + convCmd[1] + " " + convCmd[2] + " " + convCmd[3] + " "
              + convCmd[4] + "<br>\n");
        ...
        System.out.println("An error occured while convering log files ("
            + e.getMessage() + ")");
      }
      ...
      try {
        ...
        System.out.println("An error occured while generating the graphs ("
            + e.getMessage() + ")");
      }
    }
    ...
    System.out.println("<TR><TD><IMG SRC=\"cpu_busy."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_cpu_busy."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_cpu_busy."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("<TR><TD><IMG SRC=\"cpu_idle."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_cpu_idle."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_cpu_idle."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("<TR><TD><IMG SRC=\"cpu_user_kernel."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_cpu_user_kernel."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_cpu_user_kernel."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("</TABLE><p>");
    ...
    System.out.println("<TR><TD><IMG SRC=\"procs."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_procs."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_procs."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("<TR><TD><IMG SRC=\"ctxtsw."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_ctxtsw."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_ctxtsw."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("</TABLE><p>");
    ...
    System.out.println("<TABLE>");
    System.out.println("<TR><TD><IMG SRC=\"mem_usage."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_mem_usage."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_mem_usage."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("<TR><TD><IMG SRC=\"mem_cache."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_mem_cache."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_mem_cache."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("</TABLE><p>");
    ...
    System.out.println("<TABLE>");
    System.out.println("<TR><TD><IMG SRC=\"disk_rw_req."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_disk_rw_req."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_disk_rw_req."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("<TR><TD><IMG SRC=\"disk_tps."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_disk_tps."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_disk_tps."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("</TABLE><p>");
    ...
    System.out.println("<TABLE>");
    System.out.println("<TR><TD><IMG SRC=\"net_rt_byt."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_net_rt_byt."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_net_rt_byt."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("<TR><TD><IMG SRC=\"net_rt_pack."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_net_rt_pack."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_net_rt_pack."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("<TR><TD><IMG SRC=\"socks."
        + client.rubbos.getGnuPlotTerminal()
        + "\"><TD><IMG SRC=\"client_socks."
        + client.rubbos.getGnuPlotTerminal() + "\">");
    if (cjdbcFlag) {
      System.out.println("<TR><TD><IMG SRC=\"cjdbc_server_socks."
          + client.rubbos.getGnuPlotTerminal() + "\">");
    }
    System.out.println("</TABLE><p>");
    ...
    if (isMainClient) {
      ...
      try {
        ...
        cmd[3] = "-v";
        cmd[4] = "nbscript="
            + Integer.toString(client.rubbos.getRemoteClients().size() + 1);
        cmd[5] = reportDir + "stat_client0.html";
        Process computeStats = Runtime.getRuntime().exec(cmd);
        // Need to read input so program does not stall.
        BufferedReader read = new BufferedReader(new InputStreamReader(
            computeStats.getInputStream()));
        String msg;
        while ((msg = read.readLine()) != null)
          System.out.println(msg + "<br>");
        read.close();
